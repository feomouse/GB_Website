...
namespace GB_Project.Services.ShopService.ShopAPI.Application.CommandsHandler
{
    public class AddGBProductCommandHandler : IRequestHandler<AddGBProductCommand, int>
    {
      ...
      public Task<int> Handle(AddGBProductCommand request, CancellationToken cancellationToken)
      {
        ...
        GBProduct gbproduct = new GBProduct(request.ProductName, double.Parse(request.OrinPrice), double.Parse(request.Price), request.Quantity, Convert.ToDateTime(request.VailSDate), Convert.ToDateTime(request.VailEDate), request.VailTime, request.Remark, request.Detail, productType);
