<template>
  <div id="mholder__SignIn">
    <div class="auto_eight">
      <div id="mleft__place">
        <img src="../../static/imgs/spring.jpg" alt="春天到了，去踏青吧" style="width: 80%; height: 30rem; padding-top: 2rem; margin-top: 8rem;"/>
      </div>
        ...
        { 
          ...
          ManagerApi.ifViolateUser(this.UserName).then(res => {
            if(res.status != 200) {
              ...
              MerchantApi.getShops(this.$store.getters.getMerchantId).then(res => {
                if(res.body.length == 0) this.$router.push('/Merchant/CreateShop');
                else {
                  this.$store.dispatch('commitSetMerchantShopId', res.body[0].shopId);
